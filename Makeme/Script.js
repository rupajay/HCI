...
const getUser = async (req, res, next) => {
  ...
  if (userEmail) {
    try {
      ...
      req.user = null; // Handle database query errors
    }
  } else {
    req.user = null; // No userEmail cookie
  }
  next();
};
...
app.post('/whishlist/add', async (req, res) => { // Reverted route
  if (!req.user) {
    return res.status(401).json({ message: 'Unauthorized. Please log in.' });
  }
  ...
  try {
    await db.query('INSERT INTO whishlist (user_id, name, price) VALUES ($1, $2, $3)', [userId, name, price]); // Reverted table name
    res.redirect('/whishlist'); // Reverted route
  } catch (error) {
...
app.get('/whishlist', async (req, res) => { // Reverted route
  const userId = req.user.id;

  try {
    const result = await db.query('SELECT * FROM whishlist WHERE user_id = $1', [userId]); // Reverted table name
    const total = await db.query('SELECT SUM(price) FROM whishlist WHERE user_id = $1', [userId]); // Reverted table name
    const sum = total.rows[0].sum || 0;

    res.render('WHISHLIST.ejs', { items: result.rows, sum });
  } catch (error) {
...
app.post('/whishlist/remove', async (req, res) => { // Reverted route
  const { id } = req.body;
  const userId = req.user.id;

  try {
    await db.query('DELETE FROM whishlist WHERE id = $1 AND user_id = $2', [id, userId]); // Reverted table name
    res.redirect('/whishlist'); // Reverted route
  } catch (error) {
...
// Example: Add functionality for live chat button
...
app.get('/track', (req, res) => {
  ...
  res.render('tracking.ejs', { userEmail });
});
